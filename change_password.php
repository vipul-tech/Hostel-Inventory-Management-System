...
if (count($_POST) > 0) {
    $result = mysqli_query($conn, "SELECT * from login_student WHERE username='" . $_SESSION["uname"] . "'");
    $row = mysqli_fetch_array($result);
    if ($_POST["currentPassword"] == $row['password']) {
        mysqli_query($conn, "UPDATE login_student set password='" . $_POST["newPassword"] . "' WHERE username='" . $_SESSION["uname"] . "'");
